...
// CSV/XLSX 업로드 → Supabase에 업서트
// 필요 env:
//   NEXT_PUBLIC_SUPABASE_URL=...
//   SUPABASE_SERVICE_ROLE_KEY=...   (서버 전용! 클라에 노출 금지)
...
function rowsFromXLSX(buf: Buffer): Raw[] {
  // 주의: .xls(아주 옛 포맷)는 환경에 따라 읽기 실패할 수 있음 → 가능하면 .xlsx/CSV 사용 권장
...
function buildKey(t: any, idx: number) {
  if (t.rowkey) return norm(t.rowkey);
  const base = [
    norm(t.docno),
    norm(t.lineno),
    norm(t.txdate),
...
export async function POST(req: NextRequest) {
  try {
    ...
    const raw: Raw[] = isXlsx ? rowsFromXLSX(buf) : rowsFromCSV(buf);
    ...
    for (let i = 0; i < rows.length; i += 1000) {
      ...
      const { data, error } = await supabase
        ...
        .select(); // 일부 버전에서는 인자 1개만 허용 → count는 data.length로 계산
